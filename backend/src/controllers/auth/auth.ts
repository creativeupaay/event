...
export const registerOrganization = async (
    ...
    const { email, password,name, designation, address, organizationType } = req.body;
    ...
    try {
        ...
        if (existingOrganization) {
            return next(new AppError("Organizartion already exists", StatusCodes.CONFLICT));
        }

        // const password = crypto.randomBytes(4).toString("hex");
        const hashedPassword = await bcrypt.hash(password, 10);

        const newOrganization = await OrganizationModel.create({ email, password: hashedPassword, designation,name, address, organizationType });
        if (!newOrganization)
        ...
        await OrganizationModel.findByIdAndUpdate(newOrganization.id, { refreshToken });
        ...
        next(
            (err instanceof Error ? err : new AppError("Error registering user", StatusCodes.INTERNAL_SERVER_ERROR))
        );
    }
};
...
export const login = async (
    ...
    if (role === Roles.ORGANIZATION) {
        ...
        if (!user)
            throw new AppError("Email is not registerd", 404);
        ...
        if (!user)
            throw new AppError("Email is not registed", 404);
